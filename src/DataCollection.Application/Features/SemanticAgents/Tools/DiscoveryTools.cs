...
)
{
    ...
    public async Task<string> SearchTextPatterns(
    ...
    )
    {
        try
        {
            ...
            {
                "bug_list" => await configurationService.GetArrayAsync<string>(
                    ConfigurationCategories.Keywords,
                    ConfigurationKeys.BugListKeywords
                ),
                "artifact" => await configurationService.GetArrayAsync<string>(
                    ConfigurationCategories.Keywords,
    ...
    )
    {
        try
        {
            var results = JsonSerializer.Deserialize<List<DiscoveryResult>>(resultsJson) ?? [];
            var validations = string.IsNullOrEmpty(validationJson)
                ? new Dictionary<string, Dictionary<string, object>>()
                : JsonSerializer
                    .Deserialize<List<Dictionary<string, object>>>(validationJson)
    ...
    {
        var keywords = new List<string>();
        var text = $"{repo.Title} {repo.Snippet}".ToLowerInvariant();
    ...
    {
        var confidence = 0.4; // Base confidence for web results
        ...
        if (
            lowerTitle.Contains("bug")
            || lowerTitle.Contains("defect")
            || lowerTitle.Contains("vulnerability")
        )
            confidence += 0.3;
        ...
        if (
            ...
            || url.Contains("zenodo.org")
            || url.Contains("figshare.com")
        )
            confidence += 0.1;
    ...
    {
        var confidence = 0.6; // Higher base confidence for Zenodo (research platform)
