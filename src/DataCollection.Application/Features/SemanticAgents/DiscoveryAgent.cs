...
) : IDiscoveryAgent
{
            ...
            foreach (var action in readyActions)
            {
                try
                {
                    ...
                    if (action.ActionType.Contains("Search") || action.ActionType.Contains("Score"))
                    {
                        try
                        {
                            var parsedResults = JsonSerializer.Deserialize<List<DiscoveryResult>>(
                                result
                            );
                ...
                catch (Exception ex)
                {
                    ...
                    actionResults[action.Id] = JsonSerializer.Serialize(new { error = ex.Message });
    ...
    )
    {
        ...
        try
        {
            ...
            var reflection = JsonSerializer.Deserialize<AgentReflection>(
                reflectionJson,
                new JsonSerializerOptions { PropertyNameCaseInsensitive = true }
            );
    ...
    )
    {
        ...
        {
            "searchtextpatterns" => await discoveryTools.SearchTextPatterns(
            ...
            _ => JsonSerializer.Serialize(new { error = $"Unknown tool: {action.Tool}" }),
        };
    }
