...
) : IBatchCriterion<TProfile, TOutcome>
{
    ...
    private readonly string apiKey = credentialOptions.Value.OpenAIToken;
    ...
    private async Task<string> UploadBatchFileAsync(List<BatchRequest> requests)
    {
        ...
        foreach (var request in requests)
        {
            ...
            var requestJson = JsonSerializer.Serialize(
                batchRequestModel,
                AppJsonContext.Default.BatchRequestModel
            );
    ...
    private async Task<string> CreateBatchJobAsync(string inputFileId)
    {
        ...
        var json = JsonSerializer.Serialize(
            requestBody,
            AppJsonContext.Default.CreateBatchJobRequest
        );
        ...
        var batchResponse = JsonSerializer.Deserialize(
            responseContent,
            AppJsonContext.Default.BatchJobResponse
        );
    ...
    )
    {
        ...
        while ((line = await reader.ReadLineAsync()) != null)
        {
            ...
            try
            {
                var batchResponse = JsonSerializer.Deserialize(
                    line,
                    AppJsonContext.Default.BatchResponse
                );
